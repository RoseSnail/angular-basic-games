...
export class App {
  ...
  collisionCheck(){
    ...
    this.shipCollisions(this.shipsB);
    this.shipCollisions(this.shipsX);
    this.shipCollisions(this.shipsY);
  }
      ...
      ships.forEach( other =>{
        if(ship.id != other.id){
          ...
          if( distance <= ship.radius() + other.radius() ){
            if( showConsole ){ console.log( ship.name + " collision!" ); }
          }
        }
      })
    });
